...
fn get_scriptpubkey(psbt: &Psbt, index: usize) -> Result<&Script, InputError> {
    let script_pubkey;
    let inp = psbt.inputs.get(index).expect("Invalid Input index");
    if let Some(ref witness_utxo) = inp.witness_utxo {
        ...
        script_pubkey = &non_witness_utxo
            .output
            .get(vout as usize)
...
fn get_descriptor(psbt: &Psbt, index: usize) -> Result<Descriptor<PublicKey>, InputError> {
    ...
    if script_pubkey.is_p2pk() {
        ...
        match bitcoin::PublicKey::from_slice(&pk_bytes[1..script_pubkey_len - 1]) {
            Ok(pk) => return Ok(Descriptor::Pk(pk)),
            Err(e) => return Err(InputError::from(e)),
        }
    } else if script_pubkey.is_p2pkh() {
        // 1. `Pkh`: creates a `PkH` descriptor if partial_sigs has the correponding pk
        ...
        match partial_sig_contains_pk {
            Some((pk, _sig)) => {
                return Ok(Descriptor::Pkh(pk.to_owned()));
            }
            None => {
        ...
        match partial_sig_contains_pk {
            Some((pk, _sig)) => {
                return Ok(Descriptor::Wpkh(pk.to_owned()));
            }
            None => {
                return Err(InputError::MissingPubkey);
            }
        ...
        if let Some(ref witness_script) = inp.witness_script {
            ...
            return Ok(Descriptor::Wsh(ms));
        } else {
            return Err(InputError::MissingWitnessScript);
        }
        ...
        if redeem_script.is_v0_p2wsh() {
            ...
            match partial_sig_contains_pk {
                Some((pk, _sig)) => {
                    return Ok(Descriptor::ShWpkh(pk.to_owned()));
                }
                None => return Err(InputError::MissingPubkey),
            }
        } else {
            //7. regular p2sh
    ...
    for (index, input) in psbt.inputs.iter().enumerate() {
        ...
        let sighash = match des {
            ...
            | Descriptor::ShWpkh(..) => {
                ...
                Segwitv0::sighash(&psbt.global.unsigned_tx, &des, index, amt, sighash_ty)
            }
            _ => Legacy::sighash(&psbt.global.unsigned_tx, &des, index, sighash_ty),
        };
